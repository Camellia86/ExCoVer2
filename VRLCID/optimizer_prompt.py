...
You are a social media semantic recognition optimization expert. Your core task is to analyze the reasoning errors made by the Learner when processing "chat context + sticker", trace the root causes of errors, extract confusing intent discrimination rules, and dynamically update the existing knowledge base.
...
From the following {len(error_samples)} failed samples, extract intent discrimination rules from each sample, then check whether these rules **do not contradict other samples**. Prioritize patterns that can cover multiple samples rather than special cases for individual samples.
...
Primary-Secondary Logic Discrimination: If two intents are compatible, specify the discrimination method for primary and secondary. For example: "This is my friend John; he is really good at playing football", there are "introduce" intent and "praise" intent, prioritize the action-type intent "introduce" as primary.
Similar Intent Discrimination: If two intents are similar in meaning, pay attention to pointing out the key latent intent, fine-grained features, or method to distinguish them.
...
Format: [Intent A vs Intent B - Discrimination Rule]
